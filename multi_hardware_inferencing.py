'''This file contains code for performing inferencing on multi-hardware
   trained model. The user can inference on hardware of their choice.'''
...
if dataset_str == 'digits01' or dataset_str == 'digits89':
    ...
    for digit in target_labels:
        digit_indices = np.where(labels_filtered == digit)[0][:]  # Get the first 50 indices for each digit
...
subset_noise_model, custom_backend = create_subset_backend(layout_qubits, layout_coupling, backend)
...
# Perform inferencing (We take mean inferencing accuracy for 10 runs)
test_acc_list = []
for i in range(10):
    test_acc = 0
    for i, (inputs, labels) in enumerate(tqdm(test_loader)):
        ...
        # print(soft_outputs,pred, labels)
        dist = torch.abs(labels - pred)
        test_acc += len(dist[dist==0])
    print(f"Test acc: {test_acc/len(X_test)*100:.2f}%\n")
    test_acc_list.append(test_acc/len(X_test)*100)

print(f'Mean Test accuracy: {sum(test_acc_list)/len(test_acc_list):.4f}')